...
2. 🔒 **Лимит — 2 заявки «на всё»** (`APP_LIMIT`). Слот участника занимают и **свои поданные**
   ...
   `slotsUsed(apps, email)` (предикат `occupiesSlot`); проверяется в **двух** точках — при подаче
...
14. 🔒 **Старший участник (35+) — только в команде.** Самостоятельно участвовать может только до 35
    ...
    клиенте: регистрация (`vDobRegister`), редирект формы для 35+, блок подачи при `over`. **Точная
    формула доли и округление — открытый вопрос** (10% или 20%?), см. [раздел 13](#13-открытые-вопросы-и-что-стоит-уточнить-у-заказчика).
...
- **Форматы/лимиты номинаций** в коде (`NOMINATIONS`) расходятся с официальным Положением
  (`docs/polozheniye.html`): у визуала JPEG/PNG vs PDF/MP4/JPG, у синтеза подкаст MP3 vs только MP4 и
  т.д. Какой источник истины? Свести значения.
- **Возрастная квота 35+ в команде:** доля старших (`SENIOR_SHARE`) и правило округления (`floor`) —
  заглушка `0.2`. Точная формула и поведение на границе (например, можно ли 1 старшего в команде из
  3?) — уточнить у заказчика.
...
**Несовершеннолетние** — ⏳ _полноценный приём и модерация согласий отложены на отдельный этап (позже)._
В **этой** фазе участник 14–17 после подтверждения почты попадает на стену `minor-wall` и **ждёт**:
приём согласий откроется отдельно, об этом сообщат на почту (текст стены под это поправить — убрать
обещание загрузки документа здесь и сейчас). Интерактив согласия проектируем отдельным этапом.
...
Колонка **«Решение»** — за тобой/командой; впишите выбранный вариант.

| # | Наша модель (прототип) | Их API сейчас | Варианты | Решение |
|---|---|---|---|---|
| 1 | Приглашение по email → ожидание → принять/отклонить (`respond-invite`, экран `/join/:id`), теги `in/confirmed/invited/declined` | Только `team_member_ids` — добавление **готовых** юзеров по id (поиск через `/users/search?email`). Инвайта/акцепта нет | (а) урезать до «добавить найденного по email», без accept-флоу и `/join`; (б) их бэк добавляет invite/accept-эндпоинты | 🟡 **Команды нужны.** На тест-фазу — обход «**капитан-онли**»: регистрируется только капитан, участников вписывает вручную (имя/роль), без аккаунтов/инвайтов. Деградация «по email-поиску» **отклонена — ПДн** (раскрывает, кто зарегистрирован; добавляет без согласия). Бэку: поле `team_members` `{name, role}` или на тест нести состав в `annotation` (`team_member_ids` пустой). Полный инвайт+accept — следующим этапом. См. [§15.4](#154-решения-по-командам-и-синтезу) |
| 2 | Отзыв (`withdraw-app`), «Исправить» (`reopen-app`), выход из команды (`leave-team`) | Эндпоинтов нет (есть только admin-смена статуса) | (а) их бэк добавляет withdraw/reopen/leave; (б) убрать кнопки из UI | ✅ **Убираем из UI** отзыв поданной и выход из команды — подача финальна в этой фазе. **Доработка (`rework`/«Исправить») отложена на этап кабинетов админа/экспертов** — сейчас не строим. Удаление черновика оставить (у их API тоже нет — мелкий `DELETE`) |
| 3 | Синтез: `synthDirs` (≥2 направления), правила вокруг них | Поля под направления в `draft` нет | (а) их бэк добавляет поле (напр. `synth_directions`) + правило ≥2; (б) синтез без под-направлений | 🟡 **На запуск — синтез без под-направлений** (`nomination = synth`). Направления аддитивны (легко добавить позже) и **нужны на будущее — роутинг заявок к экспертам по направлениям**. Рекомендация: попросить бэк добавить поле `synth_directions` (массив) **уже сейчас** (копить данные, без бэкафилла); иначе разметить вручную на малом тест-батче. См. [§15.4](#154-решения-по-командам-и-синтезу) |
| 4 | **Общий** лимит 2 «на всё» (`slotsUsed`, включая членство в команде по приглашению) | **Раздельные** бюджеты `solo_max`/`team_max` (`/applications/limits`) | (а) принять их раздельную модель (переписать наш лимит/`slotsUsed`); (б) согласовать общий счётчик | ✅ **По-нашему** — общий счётчик 2. Бэк реализует общий лимит с учётом, что **членство в команде занимает слот с момента принятия** (см. правило 2), а не просто «считать поданные». Несложно |
| 5 | Инлайн-правка `lastName/firstName/middleName/phone/work`; есть поле `work` | `PATCH /users/me` только `phone/city/nationality`; поля `work` нет; имя/ДР не редактируются | (а) сузить наш профиль под их API, убрать `work`; (б) их бэк расширяет PATCH + добавляет `work` | ✅ **По-нашему** — бэк расширяет `PATCH /users/me` (имя, `middle_name`, `work`) и добавляет поле `work`. Несложно |
| 6 | 35+ только в команде, ≤ доли состава (`teamSeniorStat`, блокирует submit) | На бэке не отражено | (а) клиентская проверка + их бэк валидирует на submit; (б) убрать правило | ❓ _решить_ |
...
но с приоритетами — чтобы под дедлайн резать осознанно и по порядку, **не ломая запуск**.
...
- Согласия (`consents`).
- Лимит 2 + одна-на-номинацию + полнота → нужно доработать **общий счётчик** (расхождение №4).
- Подача (`submit`).
- **Синтез как номинация** — на запуск **без** под-направлений (см. [§15.4](#154-решения-по-командам-и-синтезу)).
- **Команды** — на тест-фазу через обход «капитан-онли» (см. [§15.4](#154-решения-по-командам-и-синтезу)).

### 15.2. T1 — важно, но режется деградацией (не поломкой)

| Фича | Идеал | Деградация, если не успевают |
|---|---|---|
| Команды (№1) | инвайт по email + принять/отклонить + `/join` | **капитан-онли** (состав вписывает капитан вручную) |
...
| Правило 35+ ≤ доли (№6) | клиент + серверная валидация | только клиентская проверка (или снять) |

### 15.3. T2 — отложено на следующий этап (уже решено)

Согласия несовершеннолетних (интерактив; сейчас — стена-ожидание) · доработка `rework`/«Исправить» +
кабинеты админа и экспертов · отзыв поданной / выход из команды · уведомления · экран «Итоги» ·
восстановление пароля и соц-входы (если их нет в API — отложить).

### 15.4. Решения по командам и синтезу

**Команды — «капитан-онли» на тест-фазу.** Фаза сейчас — малый объём, тест; полный инвайт-флоу
(пригласить → принять → `/join`) бэк под дедлайн не успеет. Деградация «добавить по email-поиску»
**отклонена по ПДн**: поиск чужих по email раскрывает, кто зарегистрирован, и добавляет людей без их
действия/согласия — это обработка чужих перс. данных. Обход: **регистрируется только капитан**, он
создаёт команду и **вписывает участников вручную** (имя + роль) — без их аккаунтов, приглашений и
подтверждений. Для заявочной формы это естественно (капитан подаёт за команду) и чужие ПДн не
трогает. От бэка нужно немного: либо поле `team_members` (список `{name, role}`) в заявке, либо на
время теста состав несём в `annotation` / на клиенте, а `team_member_ids` остаётся пустым.
Полноценные команды (инвайт + accept) — следующим этапом.

**Синтез — на запуск без под-направлений.** Запускаем `nomination = synth` без выбора направлений
(≥2); под-выбор в UI скрываем/упрощаем. Направления **аддитивны** — добавляются полем позже без
ломки. Они **нужны на будущее**: по направлениям потом **роутят заявки к профильным экспертам**.
Поэтому: либо просим бэк добавить крошечное поле `synth_directions` (массив строк) **уже сейчас**
(данные копятся с первого дня, без бэкафилла), либо на малом тест-батче размечаем направления
вручную. Объём мал — оба варианта приемлемы; предпочтительно поле сразу.

### 15.5. Порядок отсечения (если совсем горит — режем сверху вниз)

1. **№6** — серверная валидация 35+ → только клиентская проверка.
2. **№5** — расширенное редактирование профиля → базовое (`phone/city/nationality`).
3. **№3** — синтез-направления → синтез без под-выбора.
4. **№1** — полные команды → «капитан-онли» (обход выше).

Логика: первым режем наименее болезненное; **команды держим до последнего** (нужны) — их нижняя
точка — «капитан-онли», а не удаление.
