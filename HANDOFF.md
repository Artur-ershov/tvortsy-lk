...
| 2 | Отзыв (`withdraw-app`), «Исправить» (`reopen-app`), выход из команды (`leave-team`) | Эндпоинтов нет (есть только admin-смена статуса) | (а) их бэк добавляет withdraw/reopen/leave; (б) убрать кнопки из UI | **Две разные судьбы:** (1) доработка (`rework`/«Исправить») → **T2, точно следующая итерация** (кабинеты экспертов/админа), сейчас не строим; (2) отзыв (`withdraw-app`) + выход из команды (`leave-team`) → **🟡 план Б**, режем только если не успеваем, по умолчанию оставляем. Удаление черновика — остаётся (мелкий `DELETE`) |
...
| Отзыв/выход из команды (№2) | кнопки есть | убрать кнопки: подача финальна, членство не разорвать до конца фазы |
...
❓ **Открыто (не «решено»):** восстановление пароля — зависит от того, есть ли оно в их API; если
нет — отложить.
...
Это **план на случай нехватки времени**, а не рейтинг важности и **не список на удаление.**

Если перед запуском ясно, что всё доделать не успеваем — упрощаем фичи **в этом порядке, сверху
вниз.** Верхнюю отдаём первой: её потеря почти незаметна. До нижней (команды) доходим только в самом
крайнем случае. Ни одну фичу не вырезаем целиком — каждая просто становится проще.

Формат каждого пункта: **сейчас → что остаётся после упрощения.**

1. **Отзыв заявки и выход из команды** — *жертвуем первым, почти не больно.*
   Сейчас: есть кнопки «отозвать поданную заявку» и «выйти из команды».
   → Кнопок нет: поданная заявка становится окончательной, из команды выйти нельзя.
...
5. **Команды** — *самое болезненное, держим до последнего.*
   Сейчас: капитан приглашает участников по e-mail, они принимают или отклоняют, есть экран
   присоединения по ссылке.
   → Режим «капитан вписывает всех сам»: капитан добавляет участников вручную, без приглашений и
   подтверждений.
   Даже в этом виде команды **остаются рабочими** — просто проще. Не удаляем никогда.
